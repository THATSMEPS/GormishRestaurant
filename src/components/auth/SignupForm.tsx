...
import api, { getCuisines, Cuisine } from '../../utils/api';
...
const SignupForm: React.FC<SignupFormProps> = ({
  ...
  useEffect(() => {
    const fetchCuisines = async () => {
      try {
        ...
        const options = cuisines.map((c: Cuisine) => ({ value: c.id, label: c.cuisineName }));
  ...
  // Create refs for your InputField components
  ...
  const fullAddressRef = useRef<HTMLTextAreaElement>(null); // For multiline InputField (textarea)
  const servingRadiusRef = useRef<HTMLInputElement>(null);

  // Function to handle input focus and scroll
  // FIX: Changed 'event: FocusEvent' to 'event: Event' and then cast inside
  const handleInputFocusScroll = (event: Event) => {
    const target = event.target as HTMLElement; // Cast to HTMLElement for scrollIntoView
  ...
  // Attach focus event listeners to each input field via useEffect
  useEffect(() => {
    const refsToMonitor: (HTMLInputElement | HTMLTextAreaElement | null)[] = [
      ...
      servingRadiusRef.current,
    ];

    refsToMonitor.forEach(element => {
      if (element) {
        // No change needed here, as the function signature of handleInputFocusScroll is now compatible
        element.addEventListener('focus', handleInputFocusScroll);
      }
    });
    ...
    return () => {
      refsToMonitor.forEach(element => {
        if (element) {
          ...
          element.removeEventListener('focus', handleInputFocusScroll);
        }
      });
    };
    ...
    >
      ...
      <div className="space-y-4">
        ...
        <InputField
          ...
          min="1"
          max="10"
          ref={servingRadiusRef}
        />

        <CuisineSelector selectedCuisines={cuisines} onChange={(value: { value: string; label: string }[]) => onChange('cuisines', value)} options={cuisineOptions} />
