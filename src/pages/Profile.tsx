...
const Profile: React.FC<ProfileProps> = ({ restaurant }) => {
  ...
  const [profileData, setProfileData] = useState({
    ...
    servingRadius: restaurant.servingRadius || 5,
  ...
  // Load latest restaurant data from localStorage on mount
  useEffect(() => {
      ...
      try {
        ...
        if (parsed && parsed.id === restaurant.id) {
          ...
          setProfileData({
            ...
            image: parsed.banners?.[0] || '',
            servingRadius: parsed.servingRadius || 5,
          ...
          setOperatingHours(parsed.hours || operatingHours);
        }
      } catch (e) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const payload = {
        ...
        address: addressObj,
        cuisines: Array.isArray(profileData.cuisineTypes)
          ? profileData.cuisineTypes.map(c => c.value.trim()).join(', ')
          : typeof profileData.cuisineTypes === 'string'
        ...
        password: '******', // Send placeholder password as per user request
      };
      ...
      const normalizedCuisinesResponse = Array.isArray(response.data.cuisines)
        ...
        : [];
          ...
          setProfileData({
            ...
            address: response.data.address?.street || profileData.address,
  ...
  const handleImageUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (files) {
      ...
      // Update profileData banners with object URLs for preview
      const objectUrls = newFiles.map(file => URL.createObjectURL(file));
      setProfileData({ ...profileData, banners: objectUrls, image: objectUrls[0] || '' });
      // Removed setIsEditingImage(false) to keep edit mode active after image selection
      // setIsEditingImage(false);
    }
  };
  ...
  const handleSaveImage = async () => {
    ...
    setIsSaving(true);
    try {
      // Upload the first image file using the new API function
      const bannerUrl = await uploadBannerImage(restaurant.id, imageFiles[0]);

      // Update profileData with the new banner URL
      setProfileData(prev => ({
        ...prev,
        banners: [bannerUrl],
        image: bannerUrl,
      ...
      <div className="space-y-6">
        ...
        >
          <div className="relative h-48 sm:h-64">
            <img
              src={profileData.image || 'https://placehold.co/600x400'}
              alt="Restaurant"
        ...
        >
          ...
          <div className="space-y-4">
            ...
            <div>
              <label className="form-label" htmlFor="servingRadius">
                Serving Radius (km)
              </label>
              <div className="input-group">
        ...
        <div className="space-y-6">
          {Object.keys(operatingHours).map(day => {
            ...
            return (
              <div key={day} className="border rounded-md p-4 flex items-center justify-between">
                <div>
                ...
                <div className="flex items-center ml-4">
                  <label htmlFor={`${day}-isOpen`} className="mr-2 text-sm font-medium text-gray-700">
                    {isOpen ? 'Open' : 'Closed'}
                  </label>
                  <input
                    id={`${day}-isOpen`}
